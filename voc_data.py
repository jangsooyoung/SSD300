...
class VOCDataset():
    ...
    Attributes:
        root_dir: dataset root dir (ex: face-voc)

        num_examples: number of examples to be used
                      (in case one wants to overfit small data)
    """

    def __init__(self, root_dir, default_boxes, class_idx_to_name, new_size, num_examples=-1, augmentation=None):

        super(VOCDataset, self).__init__()
        self.idx_to_name = class_idx_to_name
        self.name_to_idx = dict([(v, k)  for k, v in enumerate(self.idx_to_name)])
        self.image_dir = root_dir
        self.anno_dir  = root_dir
        #self.ids = list(map(lambda x: x[:-4], os.listdir(self.image_dir)))
        #self.ids = list(map(lambda x: x.replace(self.image_dir + os.path.sep, '')[:-4], glob.glob(self.image_dir + os.path.sep + "*" +  os.path.sep +"*.jpg")))
        self.ids = list(map(lambda x: x[:-4],  glob.glob(self.image_dir + os.path.sep + "*" + os.path.sep + "*.jpg")))
        #self.ids = list(map(lambda x: x[:-4],  glob.glob(self.image_dir + os.path.sep + "*.jpg")))
...
def create_batch_generator(root_dir, default_boxes, class_idx_to_name,
                           new_size, batch_size, num_batches, mode,augmentation=None ):
    num_examples = batch_size * num_batches if num_batches > 0 else -1
    voc = VOCDataset(root_dir, default_boxes, class_idx_to_name, new_size, num_examples, augmentation)
